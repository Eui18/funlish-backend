...
public class AuthService {
    ...
    public UserResponseDto findUser(LoginUserDto dto) {
       ...
       User user = repository.findUser(dto.getTuition(), dto.getPassword())
        .orElseThrow(() -> new UnauthorizedException("Usuario no encontrado o credenciales inválidas"));
...
private void validateStudent(CreateUserDto dto) {
    ...
    if (!dto.getTuition().matches("\\d{6}")) {
        throw new ValidationException(List.of("La matrícula del alumno debe tener exactamente 6 dígitos."));
    }

    if (!dto.getEmail().matches("^\\d{6}@.+$")) {
        throw new ValidationException(List.of("El correo del alumno debe iniciar con la matrícula."));
    }

    String emailPrefix = dto.getEmail().split("@")[0];

    if (!emailPrefix.equals(dto.getTuition())) {
        throw new ValidationException(List.of("La matrícula no coincide con el correo institucional."));
    }
}
...
private void validateTeacher(CreateUserDto dto) {
    ...
    String emailPrefix = dto.getEmail().split("@")[0];
    String tuitionFromEmail = emailPrefix.replaceAll("[A-Za-z]+", "");

    if (!tuitionFromEmail.equals(dto.getTuition())) {
        throw new ValidationException(List.of("La matrícula no coincide con el correo institucional."));
    }
}
}